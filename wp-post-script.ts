import { PaperEvaluator } from './src/lib/paperEvaluator';
import { PaperInfo, EvaluationResult, FormattedOutput } from './src/lib/types';
import * as dotenv from 'dotenv';

// 環境変数を読み込み
dotenv.config();

interface PaperEvaluationResult {
  paper: PaperInfo;
 ...
 */
function formatContentForWordPress(results: PaperEvaluationResult[]): string {
 ...
 */
async function postToWordPress(title: string, content: string, status: string = 'draft'): Promise<PostResult> {
...
async function main(): Promise<void> {
  try {
    ...
    // 評価結果の詳細を表示
    results.forEach((result, index) => {
      console.log(`${index + 1}. ${result.paper.title} - ${result.formattedOutput.point}点`);
    });
...
export { formatContentForWordPress, postToWordPress };